public class Product {
        ...
        for (int i = 0; i < Products.length; i++){  //if product was present in the array beforehand
            if (Products[i] != null && Products[i].getId() == this.getId()){
                ...
                if (updatedQuantity > initialQuantity) { //add products if updated quantity greater
                    ...
                    finalQuantity = (updatedQuantity < 0) ? 0 : updatedQuantity;
    ...
    private int countQuantity(Product p){ //if more quantity is added than the maximum size of array, then using this to count
        int counter = 0;
    ...
    public void removeProduct(int quantity){
        ...
        int updatedQuantity = (quantity < 0) ? this.getQuantity() : this.getQuantity() - quantity;
        this.setQuantity(updatedQuantity);
        if (flag == 0){
            System.out.println("Product Not found");
        }
    ...
    private boolean ArrayIsFull(){
        ...
        if (occupiedCounter == Products.length)
            return true;
        else
            return false;
    }
